...
export default function CartSheet() {
    const { auth, sharedCart } = usePage().props as unknown as {
        auth: Auth;
        sharedCart: { count: number, total: number };
    };
            ...
            <SheetContent side="right" className="w-full bg-transparent">
                <div className="flex h-full flex-col bg-card">
                    <SheetHeader className="px-6 pt-6">
                        ...
                        <SheetDescription className="mt-1 text-sm text-[#5A583A]">Spend €1000 or more and get free shipping!</SheetDescription>
                    </SheetHeader>
                    ...
                    <div className="flex flex-1">
                        {cart.items.length === 0 ? (
                            <div className="flex flex-1 items-center justify-center space-y-6 overflow-y-auto px-6">
                                <div className="mt-10 flex flex-col items-center justify-center space-y-4 text-muted-foreground">
                                    <Icon iconNode={ShoppingCart} className="mx-auto h-20 w-20" />
